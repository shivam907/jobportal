  ...
  const [filterData, setFilterData] = React.useState();
    ...
    useEffect(() => {
        const fetchJobs = async () => {
            setLoading(true);
            const {result, jobArray, experience, location} = await memoizedFetchData(props.params.company);
            setJobs(result);
    ...
    return <Lay jobArray={jobArray} setFilterData={setFilterData} location={location} experience={experience}>{loading ? <Loader /> :filterData?.length>0?filterData :jobs}</Lay>;
};
